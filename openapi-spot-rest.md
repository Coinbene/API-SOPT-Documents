...
```
获取交易所现货全部ticker的最新成交价、买一价、卖一价和24交易量, 数据10秒更新一次
限速规则：1次/1秒
...
```
获取交易所现货指定ticker的最新成交价、买一价、卖一价和24交易量, 数据500毫米更新一次
限速规则：6次/1秒
...
### 公共接口-获取Kline数据信息
```
获取指定不同周期的Kline数据信息, 单次请求的最大数据量是200。如果您选择的开始/结束时间和时间粒度导致超过单次请求的最大数据量，您的请求将只会返回200个数据。如果您希望在更大的时间范围内获取足够精细的数据，则需要使用多个开始/结束范围进行多次请求。数据10秒更新一次
限速规则：1次/1秒
...
orderType      | string | 是 | 1:限价 2:市价 8:postOnly 9:fok 10:ios
notional      | string | 否 | 市价单买入金额
clientId      | string | 否 | 用户请求id，透传返回给用户
